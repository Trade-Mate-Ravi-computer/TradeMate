...
function MonthlyReport() {
    ...
    const loadMonthlyData = async () => {
        setLoading(true)
        try {
            const data = await axios.post('https://ec2-34-230-10-177.compute-1.amazonaws.com:8080/sales/dailyReport', {
                day: date.getDate(),
                month: currenMonth,
    ...
    const loadData = async () => {
        try {
            ...
            const data = await axios.post('https://ec2-34-230-10-177.compute-1.amazonaws.com:8080/sales/monthlyReport', {
                month: currenMonth,
                year: date.getFullYear(),
                companyName: JSON.parse(localStorage.getItem('companyName')).companyName,
                email: JSON.parse(localStorage.getItem('login')).user
