...
function Profits() {
    ...
    const loadData = async () => {
        try {
            const profits = await axios.post(
                `https://ec2-34-230-10-177.compute-1.amazonaws.com:8080/sales/profit`,
                dateState,
                {
                    headers: {
    ...
    const loadYearData = async () => {
        try {
            const response = await axios.post(
                `https://ec2-34-230-10-177.compute-1.amazonaws.com:8080/sales/byyear`,
                dateState,
                {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('login') ? JSON.parse(localStorage.getItem('login')).token : ""}`
    ...
    const loadAllReport = async () => {
        try {
            const response = await axios.post(
                'https://ec2-34-230-10-177.compute-1.amazonaws.com:8080/sales/totalsum',
                dateState,
                {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('login') ? JSON.parse(localStorage.getItem('login')).token : ""}`
